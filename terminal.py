...
def Stdin(Terminal):
    """class for representing stdin as a file."""
    @staticmethod
    def fileno():
        """simply return 1"""
        return 0
...
def Stdout(Terminal):
    """class for representing stdout as a file"""
    @staticmethod
    def fileno():
        return 1
...
def test():
    """performs a test of a terminal."""
    clock = pg.time.Clock()
    import sys

    pg.init()
    surf = pg.display.set_mode((400, 300))
    term = Terminal(surf)
    sys.stdin = term
    sys.stdout = term
    print(input())
    print("hi")
    input()
    # print(term.readline(update=True))
    # while True:
